...
RGBApixel BMP::GetPixel( int i, int j ) const
{
 ...
 {
  cerr << "EasyBMP Warning: Attempted to access non-existent pixel;" << endl
...
bool BMP::SetColor( int ColorNumber , RGBApixel NewColor )
{
 ...
 if( BitDepth != 1 && BitDepth != 4 && BitDepth != 8 )
 {
  if( EasyBMPwarnings )
  {
   cerr << "EasyBMP Warning: Attempted to change color table for a BMP object" << endl
 ...
 if( !Colors )
 {
  if( EasyBMPwarnings )
  {
   cerr << "EasyBMP Warning: Attempted to set a color, but the color table" << endl
 ...
 if( ColorNumber >= TellNumberOfColors() )
 {
  if( EasyBMPwarnings )
  {
   cerr << "EasyBMP Warning: Requested color number " 
...
{ 
 ...
 if( BitDepth != 1 && BitDepth != 4 && BitDepth != 8 )
 {
  if( EasyBMPwarnings )
  {
   cerr << "EasyBMP Warning: Attempted to access color table for a BMP object" << endl
        << "                 that lacks a color table. Ignoring request." << endl;
  }
  return Output;
 }
 if( !Colors )
 {
  if( EasyBMPwarnings )
  {
   cerr << "EasyBMP Warning: Requested a color, but the color table" << endl
        << "                 is not defined. Ignoring request." << endl;
  }
  return Output; 
 }
...
bool BMP::SetBitDepth( int NewDepth )
{
     ...
     NewDepth != 24 && NewDepth != 32 )
 {
  if( EasyBMPwarnings )
  {
   cerr << "EasyBMP Warning: User attempted to set unsupported bit depth " 
        << NewDepth << "." << endl
...
bool BMP::SetSize(int NewWidth , int NewHeight )
{
 ...
 if( NewWidth <= 0 || NewHeight <= 0 )
 {
  if( EasyBMPwarnings )
  {
   cerr << "EasyBMP Warning: User attempted to set a non-positive width or height." << endl
...
bool BMP::WriteToFile( const char* FileName )
{
 ...
 if( !EasyBMPcheckDataSize() )
 {
  if( EasyBMPwarnings )
  {
   cerr << "EasyBMP Error: Data types are wrong size!" << endl
 ...
 if( fp == NULL )
 {
  if( EasyBMPwarnings )
  {
   cerr << "EasyBMP Error: Cannot open file " 
 ...
 {  
  ...
  while( j > -1 )
  {
   ...
   if( !Success )
   {
    if( EasyBMPwarnings )
    {
     cerr << "EasyBMP Error: Could not write proper amount of data." << endl;
	}
    j = -1; 
   }
   j--; 
...
{ 
 ...
 if( bmih.biCompression == 1 || bmih.biCompression == 2 )
 {
  if( EasyBMPwarnings )
  {
   cerr << "EasyBMP Error: " << FileName << " is (RLE) compressed." << endl
 ...
 if( bmih.biCompression > 3 )
 {
  if( EasyBMPwarnings )
  {
   cerr << "EasyBMP Error: " << FileName << " is in an unsupported format." 
        << endl
        << "               (bmih.biCompression = " 
     ...
     && TempBitDepth != 24 && TempBitDepth != 32 )
 {
  if( EasyBMPwarnings )
  {
   cerr << "EasyBMP Error: " << FileName << " has unrecognized bit depth." << endl;
  }
  SetSize(1,1);
  SetBitDepth(1);
  fclose(fp);
 ...
 if( BitDepth < 16 )
 {
  ...
  if( NumberOfColorsToRead < TellNumberOfColors() )
  {
   if( EasyBMPwarnings )
   {
    cerr << "EasyBMP Warning: file " << FileName << " has an underspecified" << endl
 ...
 if( BytesToSkip > 0 && BitDepth != 16 )
 {
  if( EasyBMPwarnings )
  {
   cerr << "EasyBMP Warning: Extra meta data detected in file " << FileName << endl
 ...
 if( BitDepth != 16 )
 {
  ...
  while( j > -1 )
  {
   ...
   if( BytesRead < BufferSize )
   {
    ...
    if( EasyBMPwarnings )
    {
     cerr << "EasyBMP Error: Could not read proper amount of data." << endl;
	}
   }
   else
   {
    ...
    if( !Success )
    {
     if( EasyBMPwarnings )
     {
      cerr << "EasyBMP Error: Could not read enough pixel data!" << endl;
	 }
	 j = -1;
    }
   }   
 ...
 if( BitDepth == 16 )
 {
  ...
  if( BytesToSkip > 0 )
  {
   if( EasyBMPwarnings )
   {
    cerr << "EasyBMP Warning: Extra meta data detected in file " 
         << FileName << endl
         << "                 Data will be skipped." << endl;
   }
   ebmpBYTE* TempSkipBYTE;
...
bool BMP::CreateStandardColorTable( void )
{
 ...
 if( BitDepth != 1 && BitDepth != 4 && BitDepth != 8 )
 {
  if( EasyBMPwarnings )
  {
   cerr << "EasyBMP Warning: Attempted to create color table at a bit" << endl
...
BMFH GetBMFH( const char* szFileNameIn )
{
 ...
 if( !fp  )
 {
  if( EasyBMPwarnings )
  {
   cerr << "EasyBMP Error: Cannot initialize from file " 
        << szFileNameIn << "." << endl
...
void DisplayBitmapInfo( const char* szFileNameIn )
{
 ...
 cerr << "File information for file " << szFileNameIn 
      << ":" << endl << endl;

 cerr << "BITMAPFILEHEADER:" << endl
...
bool EasyBMPcheckDataSize( void )
{
 ...
 if( sizeof( ebmpBYTE ) != 1 )
 {
  if( EasyBMPwarnings )
  {
   cerr << "EasyBMP Error: ebmpBYTE has the wrong size (" 
 ...
 if( sizeof( ebmpWORD ) != 2 )
 {
  if( EasyBMPwarnings )
  {
   cerr << "EasyBMP Error: ebmpWORD has the wrong size (" 
 ...
 if( sizeof( ebmpDWORD ) != 4 )
 {
  if( EasyBMPwarnings )
  {
   cerr << "EasyBMP Error: ebmpDWORD has the wrong size (" 
